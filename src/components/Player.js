...
import {
  Play, SkipBack, Pause, SkipForward, ThumbsUp,
  ThumbsDown, DotsThreeOutlineVertical, SpeakerHigh, Repeat, Shuffle,
  CaretUp, Screencast, RepeatOnce
} from 'phosphor-react'
...
export default function Player({ data1, incrementId, decrementId, onChange,
  myRef, start, setDataToPass, formatDuration, pauseAudio, handleSliderChange, isPlaying,
  setIsPlaying, onClick, repeat, isRepeat, setIsRepeat, shuffleBtn, shuffle, setShuffle,
  handleCardClick, pointer, navigate, stack }) {
  const divRef = useRef();
  ...
  return (
    <>
      <audio ref={myRef} src={getDataById(data1).music} autoPlay={true}
        onCanPlay={e => { setIsPlaying(true); }} onEnded={playNext} loop={isRepeat} />
        ...
        <Box className="playerbtn">
          <Box display={"flex"} gap={2} sx={{ alignContent: "center", justifyContent: "center" }}  >
            <Stack spacing={1} sx={{ alignItems: "center" }} direction={"row"}>
              <IconButton sx={{ color: "white" }} onClick={() => navigate("prev")} >
                <SkipBack weight='fill' size={24} /></IconButton>
              {isPlaying ? (<IconButton sx={{ color: "white" }} onClick={pauseAudio}>
                <Pause size={30} weight="fill" /></IconButton>
              ) : (
                <IconButton sx={{ color: "white" }} onClick={start}> <Play size={30} weight="fill" />
                </IconButton>
              )}
              <IconButton sx={{ color: "white" }} onClick={() => navigate("next")} >
                <SkipForward size={24} weight="fill" /></IconButton>
          ...
          <Box gap={2} sx={{
            alignItems: "center",
            minWidth: "15em", display: "flex", paddingRight: 4
          }}>
            <Stack direction={"row"} spacing={1} sx={{ alignItems: "center" }}>
              <SpeakerHigh size={24} onPointerEnter={e => {
              ...
              /><Box style={style} onPointerLeave={e => {
                ...
                /></Box></Stack>
            {isRepeat ? (<IconButton sx={{ color: "white" }} onClick={repeat}>
              <RepeatOnce size={24} /></IconButton>
            ) : (
              <IconButton sx={{ color: "white" }} onClick={repeat}>
                <Repeat size={24} weight="fill" />
              </IconButton>)}
            {shuffle ? (<IconButton sx={{ color: "white" }} onClick={shuffleBtn}>
              <Shuffle size={24} weight="bold" /></IconButton>
            ) : (
              <IconButton sx={{ color: "white" }} onClick={shuffleBtn}> <Shuffle size={24} />
              </IconButton>)}
        ...
        <Box className="playerbtn2" ref={divRef} >
          <Box className="playerbtn1">
            ...
            <Stack direction={"row"} alignItems="center" paddingRight={2}>
              ...
              {isPlaying ? (<IconButton sx={{ color: "white" }} onClick={pauseAudio}>
                <Pause size={30} weight="fill" /></IconButton>) : (<IconButton sx={{ color: "white" }}
                  onClick={start}> <Play size={30} weight="fill" /></IconButton>
              )}
            </Stack>
